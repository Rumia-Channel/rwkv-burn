# K KIMI LINEAR: AN EXPRESSIVE, EFFICIENT ATTENTION ARCHITECTURE

TECHNICAL REPORT OF KIMI LINEAR

Kimi Team

Ohttps://github.com/MoonshotAI/Kimi-Linear
...
![bo_d6k05177aajc739dlkig_0_260_1350_1284_505_0.jpg](images/bo_d6k05177aajc739dlkig_0_260_1350_1284_505_0.jpg)
...
\( {}^{1} \) Ohttps://github.com/fla-org/flash-linear-attention/tree/main/fla/ops/kda

9 https://huggingface.co/moonshotai/Kimi-Linear-48B-A3B-Instruct
...
Kimi Linear interleaves KDA with periodic full attention layers in a uniform 3:1 ratio. This hybrid structure reduces memory and KV-cache usage by up to 75% during long-sequence generation while preserving global information flow via the full attention layers. Through matched-scale pretraining and evaluation, we show that Kimi Linear consistently matches or outperforms strong full-attention baselines across short-context, long-context, and RL-style post-training tasks-while achieving up to \( 6 \times \) higher decoding throughput at \( 1\mathrm{M} \) context length.
...
In this paper, we define \( {\square }_{t} \in  {\mathbb{R}}^{{d}_{k}} \) or \( {\mathbb{R}}^{{d}_{v}} \) , s.t., \( \square  \in  \{ \mathbf{q},\mathbf{k},\mathbf{v},\mathbf{o},\mathbf{u},\mathbf{w}\} \) denotes a \( t \) -th corresponding column vector, and \( {\mathbf{S}}_{t} \in  {\mathbb{R}}^{{d}_{k} \times  {d}_{v}} \) represents the matrix-form memory state. \( \mathbf{M} \) and \( {\mathbf{M}}^{ - } \) denote lower-triangular masks with and without diagonal elements, respectively; for convenience, we also write them as Tril and StrictTril.

Chunk-wise Formulation Suppose the sequence is split into \( L/C \) chunks where each chunk is of length \( C \) . We define \( {\square }_{\left\lbrack  t\right\rbrack  } \in  {\mathbb{R}}^{C \times  d} \) for \( \square  \in  \{ \mathbf{Q},\mathbf{K},\mathbf{V},\mathbf{O},\mathbf{U},\mathbf{W}\} \) are matrices that stack the vectors within the \( t \) -th chunk, and \( {\square }_{\left\lbrack  t\right\rbrack  }^{r} = {\square }_{{tC} + r} \) is the \( r \) -th element of the chunk. Note that \( t \in  \lbrack 0, L/C), r \in  \left\lbrack  {1, C}\right\rbrack \) . State matrices are also re-indexed such that \( {\mathbf{S}}_{\left\lbrack  t\right\rbrack  }^{i} = {\mathbf{S}}_{{tC} + i} \) . Additionally, \( {\mathbf{S}}_{\left\lbrack  t\right\rbrack  } \mathrel{\text{ := }} {\mathbf{S}}_{\left\lbrack  t\right\rbrack  }^{0} = {\mathbf{S}}_{\left\lbrack  t - 1\right\rbrack  }^{C} \) , i.e., the initial state of a chunk is the last state of the previous chunk.

Decay Formulation We define the cumulative decay \( {\gamma }_{\left\lbrack  t\right\rbrack  }^{i \rightarrow  j} \mathrel{\text{ := }} \mathop{\prod }\limits_{{k = i}}^{j}{\alpha }_{\left\lbrack  t\right\rbrack  }^{k} \) , and abbreviate \( {\gamma }_{\left\lbrack  t\right\rbrack  }^{1 \rightarrow  r} \) as \( {\gamma }_{\left\lbrack  t\right\rbrack  }^{r} \) . Additionally, \( {\mathcal{A}}_{\left\lbrack  t\right\rbrack  } \mathrel{\text{ := }} {\mathcal{A}}_{\left\lbrack  t\right\rbrack  }^{i/j} \in  {\mathbb{R}}^{C \times  C} \) is the matrix with elements \( {\gamma }_{\left\lbrack  t\right\rbrack  }^{i}/{\gamma }_{\left\lbrack  t\right\rbrack  }^{j} \) . Diag \( \left( {\mathbf{\alpha }}_{t}\right) \) denotes the fine-grained decay, Diag \( \left( {\mathbf{\gamma }}_{\left\lbrack  t\right\rbrack  }^{i \rightarrow  j}\right)  \mathrel{\text{ := }} \; \mathop{\prod }\limits_{{k = i}}^{j}\operatorname{Diag}\left( {\mathbf{\alpha }}_{\left\lbrack  t\right\rbrack  }^{k}\right) \) , and \( {\mathbf{\Gamma }}_{\left\lbrack  t\right\rbrack  }^{i \rightarrow  j} \in  {\mathbb{R}}^{C \times  {d}_{k}} \) is the matrix stack from \( {\mathbf{\gamma }}_{\left\lbrack  t\right\rbrack  }^{i} \) to \( {\mathbf{\gamma }}_{\left\lbrack  t\right\rbrack  }^{j} \) .
...
\[
{\mathbf{S}}_{t} = {\mathbf{S}}_{t - 1} + {\mathbf{k}}_{t}{\mathbf{v}}_{t}^{\top },\;{\mathbf{o}}_{t} = {\mathbf{S}}_{t}^{\top }{\mathbf{q}}_{t}.
\]

From the fast-weight perspective \( \left\lbrack  {{84},{85}}\right\rbrack  ,{\mathbf{S}}_{t} \) serves as an associative memory storing transient mappings from keys to values. This update can be viewed as performing gradient descent on the unbounded correlation objective

\[
{\mathcal{L}}_{t}\left( \mathbf{S}\right)  =  - \left\langle  {{\mathbf{S}}^{\top }{\mathbf{k}}_{t},{\mathbf{v}}_{t}}\right\rangle  ,
\]
...
\[
{\mathcal{L}}_{t}\left( \mathbf{S}\right)  = \frac{1}{2}{\begin{Vmatrix}{\mathbf{S}}^{\top }{\mathbf{k}}_{t} - {\mathbf{v}}_{t}\end{Vmatrix}}^{2}.
\]

Taking a gradient step with learning rate \( {\beta }_{t} \) gives

\[
{\mathbf{S}}_{t} = {\mathbf{S}}_{t - 1} - {\beta }_{t}{\nabla }_{\mathbf{S}}{\mathcal{L}}_{t}\left( {\mathbf{S}}_{t - 1}\right)  = \left( {\mathbf{I} - {\beta }_{t}{\mathbf{k}}_{t}{\mathbf{k}}_{t}^{\top }}\right) {\mathbf{S}}_{t - 1} + {\beta }_{t}{\mathbf{k}}_{t}{\mathbf{v}}_{t}^{\top }.
\]

This rule-the classical delta rule-treats \( \mathbf{S} \) as a learnable associative memory that continually corrects itself toward the mapping \( {\mathbf{k}}_{t} \mapsto  {\mathbf{v}}_{t} \) . The rank-1 update structure, equivalent to a generalized Householder transformation, supports hardware-efficient chunkwise parallelization [11, 112].

Gated DeltaNet as Weight Decay. Although DeltaNet stabilizes learning, it still retains outdated associations indefinitely. Gated DeltaNet (GDN) [111] introduces a scalar forget gate \( {\alpha }_{t} \in  \left\lbrack  {0,1}\right\rbrack \) , yielding

\[
{\mathbf{S}}_{t} = {\alpha }_{t}\left( {\mathbf{I} - {\beta }_{t}{\mathbf{k}}_{t}{\mathbf{k}}_{t}^{\top }}\right) {\mathbf{S}}_{t - 1} + {\beta }_{t}{\mathbf{k}}_{t}{\mathbf{v}}_{t}^{\top }.
\]
...
\( {}^{3} \) When the state transformation matrix preserves its orthogonality, absolute positional encodings can also be applied independently to \( \mathbf{q} \) and \( \mathbf{k} \) to be converted into relative positional encodings during the attention computation [87].

---

## 3 Kimi Delta Attention: Improving Delta Rule with Fine-grained Gating

We propose Kimi Delta Attention (KDA), a new gated linear attention variant that refines GDN's scalar decay by introducing a fine-grained diagonalized gate \( \operatorname{Diag}\left( {\mathbf{\alpha }}_{t}\right) \) that enables fine-grained control over memory decay and positional awareness (as discussed in §6.1). We begin by introducing the chunkwise parallelization of KDA, showing how a series of rank-1 matrix transformations can be compressed into a dense representation while maintaining stability under diagonal gating. We then highlight the efficiency gains of KDA over the standard DPLR (Diagonal-Plus-Low-Rank) formulation [30, 71].

\[
{\mathbf{S}}_{t} = \left( {\mathbf{I} - {\beta }_{t}{\mathbf{k}}_{t}{\mathbf{k}}_{t}^{\top }}\right) \operatorname{Diag}\left( {\mathbf{\alpha }}_{t}\right) {\mathbf{S}}_{t - 1} + {\beta }_{t}{\mathbf{k}}_{t}{\mathbf{v}}_{t}^{\top } \in  {\mathbb{R}}^{{d}_{k} \times  {d}_{v}};\;{\mathbf{o}}_{t} = {\mathbf{S}}_{t}^{\top }{\mathbf{q}}_{t} \in  {\mathbb{R}}^{{d}_{v}} \tag{1}
\]

![bo_d6k05177aajc739dlkig_3_269_620_1258_126_0.jpg](images/bo_d6k05177aajc739dlkig_3_269_620_1258_126_0.jpg)
...
\[
{\mathbf{S}}_{\left\lbrack  t\right\rbrack  }^{r} = \underset{ \mathrel{\text{ := }} {\mathbf{P}}_{\left\lbrack  t\right\rbrack  }^{r}}{\underbrace{\left( \mathop{\prod }\limits_{{i = 1}}^{r}\left( \mathbf{I} - {\beta }_{\left\lbrack  t\right\rbrack  }^{i}{k}_{\left\lbrack  t\right\rbrack  }^{i}{k}_{\left\lbrack  t\right\rbrack  }^{i \intercal  }\right) \operatorname{Diag}\left( {\mathbf{\alpha }}_{\left\lbrack  t\right\rbrack  }^{i}\right) \right) }} \cdot  {\mathbf{S}}_{\left\lbrack  t\right\rbrack  }^{0} + \underset{ \mathrel{\text{ := }} {\mathbf{P}}_{\left\lbrack  t\right\rbrack  }^{r}}{\underbrace{\mathop{\sum }\limits_{{i = 1}}^{r}\left( {\mathop{\prod }\limits_{{j = i + 1}}^{r}\left( {\mathbf{I} - {\beta }_{\left\lbrack  t\right\rbrack  }^{j}{k}_{\left\lbrack  t\right\rbrack  }^{j}{k}_{\left\lbrack  t\right\rbrack  }^{j \intercal  }}\right) \operatorname{Diag}\left( {\mathbf{\alpha }}_{\left\lbrack  t\right\rbrack  }^{j}\right) }\right) }} \cdot  {\beta }_{\left\lbrack  t\right\rbrack  }^{i}{k}_{\left\lbrack  t\right\rbrack  }^{i}{v}_{\left\lbrack  t\right\rbrack  }^{i \intercal  } \tag{2}
\]
...
\[
{\mathbf{P}}_{\left\lbrack  t\right\rbrack  }^{r} = \operatorname{Diag}\left( {\gamma }_{\left\lbrack  t\right\rbrack  }^{r}\right)  - \mathop{\sum }\limits_{{i = 1}}^{r}\operatorname{Diag}\left( {\gamma }_{\left\lbrack  t\right\rbrack  }^{i \rightarrow  r}\right) {\mathbf{k}}_{\left\lbrack  t\right\rbrack  }^{i}{\mathbf{w}}_{\left\lbrack  t\right\rbrack  }^{i\top }\;{\mathbf{H}}_{\left\lbrack  t\right\rbrack  }^{r} = \mathop{\sum }\limits_{{i = 1}}^{t}\operatorname{Diag}\left( {\gamma }_{\left\lbrack  t\right\rbrack  }^{i \rightarrow  r}\right) {\mathbf{k}}_{\left\lbrack  t\right\rbrack  }^{i}{\mathbf{u}}_{\left\lbrack  t\right\rbrack  }^{i\top } \tag{3}
\]

where the auxiliary vector \( {\mathbf{w}}_{t} \in  {\mathbb{R}}^{{d}_{k}} \) and \( {\mathbf{u}}_{t} \in  {\mathbb{R}}^{{d}_{v}} \) are computed via the following recurrence relation:

\[
{\mathbf{w}}_{\left\lbrack  t\right\rbrack  }^{r} = {\beta }_{\left\lbrack  t\right\rbrack  }^{r}\left( {\operatorname{Diag}\left( {\mathbf{\gamma }}_{\left\lbrack  t\right\rbrack  }^{r}\right) {\mathbf{k}}_{\left\lbrack  t\right\rbrack  }^{r} - \mathop{\sum }\limits_{{i = 1}}^{{r - 1}}{\mathbf{w}}_{\left\lbrack  t\right\rbrack  }^{i}\left( {{\mathbf{k}}_{\left\lbrack  t\right\rbrack  }^{i\top }\operatorname{Diag}\left( {\mathbf{\gamma }}_{\left\lbrack  t\right\rbrack  }^{i \rightarrow  r}\right) {\mathbf{k}}_{\left\lbrack  t\right\rbrack  }^{r}}\right) }\right) \tag{4}
\]

\[
{\mathbf{u}}_{\left\lbrack  t\right\rbrack  }^{r} = {\beta }_{\left\lbrack  t\right\rbrack  }^{r}\left( {{\mathbf{v}}_{\left\lbrack  t\right\rbrack  }^{r} - \mathop{\sum }\limits_{{i = 1}}^{{r - 1}}{\mathbf{u}}_{\left\lbrack  t\right\rbrack  }^{i}\left( {{\mathbf{k}}_{\left\lbrack  t\right\rbrack  }^{i\top }\operatorname{Diag}\left( {\gamma }_{\left\lbrack  t\right\rbrack  }^{i \rightarrow  r}\right) {\mathbf{k}}_{\left\lbrack  t\right\rbrack  }^{r}}\right) }\right) \tag{5}
\]
...
\[
{\mathbf{M}}_{\left\lbrack  t\right\rbrack  } = {\left( \mathbf{I} + \operatorname{StrictTril}\left( \operatorname{Diag}\left( {\beta }_{\left\lbrack  t\right\rbrack  }\right) \left( {\mathbf{\Gamma }}_{\left\lbrack  t\right\rbrack  }^{1 \rightarrow  C} \odot  {\mathbf{K}}_{\left\lbrack  t\right\rbrack  }\right) {\left( \frac{{\mathbf{K}}_{\left\lbrack  t\right\rbrack  }}{{\mathbf{\Gamma }}_{\left\lbrack  t\right\rbrack  }^{1 \rightarrow  C}}\right) }^{\top }\right) \right) }^{-1}\operatorname{Diag}\left( {\beta }_{\left\lbrack  t\right\rbrack  }\right) \tag{6}
\]

\[
{\mathbf{W}}_{\left\lbrack  t\right\rbrack  } = {\mathbf{M}}_{\left\lbrack  t\right\rbrack  }\left( {{\mathbf{\Gamma }}_{\left\lbrack  t\right\rbrack  }^{1 \rightarrow  C} \odot  {\mathbf{K}}_{\left\lbrack  t\right\rbrack  }}\right) ,\;{\mathbf{U}}_{\left\lbrack  t\right\rbrack  } = {\mathbf{M}}_{\left\lbrack  t\right\rbrack  }{\mathbf{V}}_{\left\lbrack  t\right\rbrack  } \tag{7}
\]
...
\[
{\mathbf{S}}_{\left\lbrack  t + 1\right\rbrack  } = \operatorname{Diag}\left( {\gamma }_{\left\lbrack  t\right\rbrack  }^{C}\right) {\mathbf{S}}_{\left\lbrack  t\right\rbrack  } + {\left( {\mathbf{\Gamma }}_{\left\lbrack  t\right\rbrack  }^{i \rightarrow  C} \odot  {\mathbf{K}}_{\left\lbrack  t\right\rbrack  }\right) }^{\top }\left( {{\mathbf{U}}_{\left\lbrack  t\right\rbrack  } - {\mathbf{W}}_{\left\lbrack  t\right\rbrack  }{\mathbf{S}}_{\left\lbrack  t\right\rbrack  }}\right)  \in  {\mathbb{R}}^{{d}_{k} \times  {d}_{v}} \tag{8}
\]
...
\[
{\mathbf{O}}_{\left\lbrack  t\right\rbrack  } = \underset{\text{ inter chunk }}{\underbrace{\left( {{\mathbf{\Gamma }}_{\left\lbrack  t\right\rbrack  }^{1 \rightarrow  C} \odot  {\mathbf{Q}}_{\left\lbrack  t\right\rbrack  }}\right) {\mathbf{S}}_{\left\lbrack  t\right\rbrack  }}} + \underset{\text{ intra chunk }}{\underbrace{\operatorname{Tril}\left( {\left( {{\mathbf{\Gamma }}_{\left\lbrack  t\right\rbrack  }^{1 \rightarrow  C} \odot  {\mathbf{Q}}_{\left\lbrack  t\right\rbrack  }}\right) {\left( \frac{{\mathbf{K}}_{\left\lbrack  t\right\rbrack  }}{{\mathbf{\Gamma }}_{\left\lbrack  t\right\rbrack  }^{1 \rightarrow  C}}\right) }^{\top }}\right) }}\underset{\text{ “peudo” -value term }}{\underbrace{\left( {\mathbf{U}}_{\left\lbrack  t\right\rbrack  } - {\mathbf{W}}_{\left\lbrack  t\right\rbrack  }{\mathbf{S}}_{\left\lbrack  t\right\rbrack  }\right) }} \in  {\mathbb{R}}^{C \times  {d}_{v}} \tag{9}
\]

![bo_d6k05177aajc739dlkig_4_579_703_739_115_0.jpg](images/bo_d6k05177aajc739dlkig_4_579_703_739_115_0.jpg)

### 3.2 Efficiency Analysis

In terms of representational capacity, KDA aligns with the generalized DPLR formulation, i.e., \( {\mathbf{S}}_{t} = \left( {\mathbf{D} - {\mathbf{a}}_{t}{\mathbf{b}}_{t}^{\top }}\right) {\mathbf{S}}_{t - 1} + {\mathbf{k}}_{t}{\mathbf{v}}_{t}^{\top } \) , both exhibiting fine-grained decay behavior. However, such fine-grained decay introduces numerical precision issues during division operations (e.g., the intra-chunk computation in Eq. 9). To address this, prior work such as GLA [114] performs computations in the logarithmic domain and introduces secondary chunking in full precision. This approach, however, prevents full utilization of half-precision matrix multiplications and significantly reduces operator speed. By binding both variables \( \mathbf{a} \) and \( \mathbf{b} \) to \( \mathbf{k} \) , KDA effectively alleviates this bottleneck-reducing the number of second-level chunk matrix computations from four to two, and further eliminating three additional matrix multiplications. As a result, the operator efficiency of KDA improves by roughly 100% compared to the DPLR formulation. A detailed analysis is provided in §6.2.

![bo_d6k05177aajc739dlkig_4_1119_934_474_356_0.jpg](images/bo_d6k05177aajc739dlkig_4_1119_934_474_356_0.jpg)
...
Neural Parameterization Let \( {\mathbf{x}}_{t} \in  {\mathbb{R}}^{d} \) be the \( t \) -th token input representation, the input to KDA for each head \( h \) is computed as follows

\[
{\mathbf{q}}_{t}^{h},{\mathbf{k}}_{t}^{h} = \operatorname{L2Norm}\left( {\operatorname{Swish}\left( {\operatorname{ShortConv}\left( {{\mathbf{W}}_{q/k}^{h}{\mathbf{x}}_{t}}\right) }\right) }\right)  \in  {\mathbb{R}}^{{d}_{k}}
\]

\[
{\mathbf{v}}_{t}^{h} = \operatorname{Swish}\left( {\operatorname{ShortConv}\left( {{\mathbf{W}}_{v}^{h}{\mathbf{x}}_{t}}\right) }\right)  \in  {\mathbb{R}}^{{d}_{v}}
\]

\[
{\mathbf{\alpha }}_{t}^{h} = f\left( {{\mathbf{W}}_{\alpha }^{ \uparrow  }{\mathbf{W}}_{\alpha }^{ \downarrow  }{\mathbf{x}}_{t}}\right)  \in  {\left\lbrack  0,1\right\rbrack  }^{{d}_{k}}
\]

\[
{\beta }_{t}^{h} = \operatorname{Sigmoid}\left( {{\mathbf{W}}_{\beta }^{h}{\mathbf{x}}_{t}}\right)  \in  \left\lbrack  {0,1}\right\rbrack
\]

where \( {d}_{k},{d}_{v} \) represent the key and value head dimensions, which are set to 128 for all experiments. For \( \mathbf{q},\mathbf{k},\mathbf{v} \) , we apply a ShortConv followed by a Swish activation, following [111]. The \( \mathbf{q} \) and \( \mathbf{k} \) representations are further normalized using L2Norm to ensure eigenvalues stability, as suggested by [112]. The per-channel decay \( {\mathbf{\alpha }}_{t}^{h} \) is parameterized via a low-rank projection \( \left( {\mathbf{W}}_{\alpha }^{ \downarrow  }\right. \) and \( {\mathbf{W}}_{\alpha }^{ \uparrow  } \) with rank equal to the head dimension) and a decay function \( f\left( \cdot \right) \) similar to those used in GDN and Mamba [111,16]. Before the output projection through \( {\mathbf{W}}_{o} \in  {\mathbb{R}}^{d \times  d} \) , we use a head-wise RMSNorm [122] and a data-dependent gating mechanism [79] parameterized as:

\[
{\mathbf{o}}_{t} = {\mathbf{W}}_{o}\left( {\operatorname{Sigmoid}\left( {{\mathbf{W}}_{g}^{ \uparrow  }{\mathbf{W}}_{g}^{ \downarrow  }{\mathbf{x}}_{t}}\right)  \odot  \operatorname{RMSNorm}\left( {\operatorname{KDA}\left( {{\mathbf{q}}_{t},{\mathbf{k}}_{t},{\mathbf{v}}_{t},{\mathbf{\alpha }}_{t},{\beta }_{t}}\right) }\right) }\right) \tag{10}
\]

![bo_d6k05177aajc739dlkig_5_387_234_1037_1037_0.jpg](images/bo_d6k05177aajc739dlkig_5_387_234_1037_1037_0.jpg)
...
No Position Encoding (NoPE) for MLA Layers. In Kimi Linear, we apply NoPE to all full attention (MLA) layers. This design delegates the entire responsibility for encoding positional information and recency bias (see § 6.1) to the KDA layers. KDA is thus established as the primary position-aware operator, fulfilling a role analogous to, or arguably stronger than, auxiliary components like short convolutions [3] or SWA [76]. Our findings align with prior results \( \left\lbrack  {{110},7,{19}}\right\rbrack \) , who similarly demonstrated that complementing global NoPE attention with a dedicated position-aware mechanism yields competitive long-context performance.

![bo_d6k05177aajc739dlkig_6_228_201_1350_740_0.jpg](images/bo_d6k05177aajc739dlkig_6_228_201_1350_740_0.jpg)
...
<table><tr><td>Input</td><td></td><td></td><td></td><td></td><td></td><td></td><td></td><td></td><td></td><td>こて</td><td>二</td><td>四四</td><td>又曰</td><td></td><td></td></tr><tr><td>Output</td><td>O O</td><td>O O</td><td>0. \( \pi \)</td><td>0.</td><td>○ ○</td><td>- 2</td><td>又</td><td></td><td>乙口</td><td></td><td></td><td></td><td></td><td></td><td>O O</td></tr></table>
...
<table><tr><td>Input</td><td></td><td></td><td></td><td></td><td></td><td></td><td>M</td><td></td><td>口</td><td></td><td></td><td></td><td><sep></td><td>B</td><td>G</td></tr><tr><td>Output</td><td></td><td></td><td></td><td></td><td></td><td></td><td>\( \phi \)</td><td></td><td></td><td></td><td></td><td></td><td>\( \phi \)</td><td>0</td><td></td></tr></table>
...
<table><tr><td></td><td></td><td>Training PPL ( \( \downarrow \) )</td><td>Validation PPL (↓)</td></tr><tr><td rowspan="5">Hybrid ratio</td><td>3:1</td><td>9.23</td><td>5.65</td></tr><tr><td>0:1</td><td>9.45</td><td>5.77</td></tr><tr><td>1:1</td><td>9.29</td><td>5.66</td></tr><tr><td>7:1</td><td>9.23</td><td>5.70</td></tr><tr><td>15:1</td><td>9.34</td><td>5.82</td></tr><tr><td colspan="2">\( w/o \) output gate</td><td>9.25</td><td>5.67</td></tr><tr><td colspan="2">\( w/s \) wish output gate</td><td>9.43</td><td>5.81</td></tr><tr><td colspan="2">w/o convolution layer</td><td>9.29</td><td>5.70</td></tr></table>
...
![bo_d6k05177aajc739dlkig_11_211_549_1385_460_0.jpg](images/bo_d6k05177aajc739dlkig_11_211_549_1385_460_0.jpg)
...
Prefilling & Decoding speed We compare the training and decoding times for full attention MLA [19], GDN-H, and Kimi Linear in Figure 7a and Figure 7b. Note that all models are based on the Kimi Linear 48B setting, with the same number of layers and attention heads. We observe that: 1) Despite incorporating a more fine-grained decay mechanism, Kimi Linear introduces negligible latency overhead compared to GDN-H during prefilling. As shown in Figure 7a, their performance curves are virtually indistinguishable, confirming that our method maintains high efficiency. The hybrid Kimi Linear model demonstrates a clear efficiency advantage over the MLA baseline as sequence length increases. While its performance is comparable to MLA at shorter lengths (4k-16k), it becomes significantly faster from 128k onwards. This efficiency gap widens dramatically at scale, with Kimi Linear outperforming MLA by a factor of 2.3 for 512k sequences and 2.9 for 1M sequences. As shown in Figure 1b, Kimi Linear fully demonstrates its advantages during the decoding phase. For decoding at \( 1\mathrm{M} \) context length, Kimi Linear is \( 6 \times \) faster than full attention.

![bo_d6k05177aajc739dlkig_12_340_199_1127_467_0.jpg](images/bo_d6k05177aajc739dlkig_12_340_199_1127_467_0.jpg)
...
\[
{s}_{t, i} = {\mathbf{q}}_{t}^{\top }\left( {\mathop{\prod }\limits_{{j = i + 1}}^{t}{\mathbf{R}}_{j}}\right) {\mathbf{k}}_{i} \tag{11}
\]

where the position relationship between the \( t \) -th query \( {\mathbf{q}}_{t} \) and the \( i \) -th key \( {\mathbf{k}}_{i} \) is reflected by the cumulative matrix products. RoPE defines the transformation matrix \( {\mathbf{R}}_{j} \) as a block diagonal matrix composed of \( {d}_{k}/2 \) 2D rotation matrices \( {\mathbf{R}}_{j}^{k} = \left( \begin{matrix} \cos \left( {j{\theta }_{k}}\right) &  - \sin \left( {j{\theta }_{k}}\right) \\  \sin \left( {j{\theta }_{k}}\right) & \cos \left( {j{\theta }_{k}}\right)  \end{matrix}\right) \) with per-2-dimensional angular frequency \( {\theta }_{k} \) . Due to the properties of rotation matrices, i.e., \( {\mathbf{R}}_{t - i} = {\mathbf{R}}_{t}^{\top }{\mathbf{R}}_{i} \) , absolute positional information \( {\mathbf{R}}_{t} \) and \( {\mathbf{R}}_{i} \) can be applied separately to \( {\mathbf{q}}_{t} \) and \( {\mathbf{k}}_{i} \) , which are then transformed into relative positional information \( t - i \) encoded as \( \mathop{\prod }\limits_{{j = i + 1}}^{t}{\mathbf{R}}_{j} = \left( \begin{matrix} \cos \left( {\left( {t - i}\right) {\theta }_{k}}\right) &  - \sin \left( {\left( {t - i}\right) {\theta }_{k}}\right) \\  \sin \left( {\left( {t - i}\right) {\theta }_{k}}\right) & \cos \left( {\left( {t - i}\right) {\theta }_{k}}\right)  \end{matrix}\right) \) . Consequently, we show that linear attentions with the gated delta rule can be expressed in a comparable formulation in Eq. 12. Similar forms for other attention variants are summarized in Table 6.

\[
{\mathbf{o}}_{t} = \mathop{\sum }\limits_{{i = 1}}^{t}\left( {{\mathbf{q}}_{t}^{\top }\left( {\mathop{\prod }\limits_{{j = i + 1}}^{t}{\mathbf{A}}_{j}\left( {\mathbf{I} - {\beta }_{j}{\mathbf{k}}_{j}{\mathbf{k}}_{j}^{\top }}\right) }\right) {\mathbf{k}}_{j}}\right) {\mathbf{v}}_{j} \tag{12}
\]
...
\( {}^{6} \) When preserving orthogonality, absolute positional encodings can be applied independently to \( \mathbf{q} \) and \( \mathbf{k} \) , which are then automatically transformed into relative positional encodings during the attention computation [87].

---

Table 6: An overview of attention mechanisms in their mathematically equivalent recurrent \( \left( {\mathbf{o}}_{t}\right) \) and parallel \( \left( \mathbf{O}\right) \) forms. We omitted the normalization term and \( {\beta }_{t} \) to achieve a more concise representation. The function \( \phi \) refers to the infinite-dimensional feature space corresponding to the exponential kernel, i.e., \( \phi {\left( \mathbf{q}\right) }^{\top }\phi \left( \mathbf{k}\right)  = \exp \left( {{\mathbf{q}}^{\top }\mathbf{k}}\right) \) .

<table><tr><td></td><td>Recurrent form</td><td>Parallel form</td></tr><tr><td></td><td>SA [99] \( \mathop{\sum }\limits_{{j = 1}}^{t}\exp \left( {{\mathbf{q}}_{t}^{\top }{\mathbf{k}}_{j}}\right) {\mathbf{v}}_{j} \)</td><td>\( \left( {\exp \left( {\mathbf{{QK}}}^{\top }\right)  \odot  \mathbf{M}}\right) \mathbf{V} \)</td></tr><tr><td>SA + RoPE [88]</td><td>\( \mathop{\sum }\limits_{{j = 1}}^{t}\exp \left( {{\mathbf{q}}_{t}^{\top }\left( {\mathop{\prod }\limits_{{s = j + 1}}^{t}{\mathbf{R}}_{s}}\right) {\mathbf{k}}_{j}}\right) {\mathbf{v}}_{j} \)</td><td>\( \left( {\exp \left( {\mathbf{R}\left( \mathbf{Q}\right) \mathbf{R}{\left( \mathbf{K}\right) }^{\top }}\right)  \odot  \mathbf{M}}\right) \mathbf{V} \)</td></tr><tr><td>LA [101]</td><td>\( \mathop{\sum }\limits_{{j = 1}}^{t}\left( {{\mathbf{q}}_{t}^{\top }{\mathbf{k}}_{j}}\right) {\mathbf{v}}_{j} \)</td><td>\( \left( {{\mathbf{{QK}}}^{\top } \odot  \mathbf{M}}\right) \mathbf{V} \)</td></tr><tr><td>Mamba2 [16]</td><td>\( \mathop{\sum }\limits_{{j = 1}}^{t}\left( {{\mathbf{q}}_{t}^{\top }\left( {\mathop{\prod }\limits_{{s = j + 1}}^{t}{\alpha }_{s}}\right) {\mathbf{k}}_{j}}\right) {\mathbf{v}}_{j} \)</td><td>\( \left( {\mathbf{Q}{\mathbf{K}}^{\top } \odot  \mathcal{A} \odot  \mathbf{M}}\right) \mathbf{V} \)</td></tr><tr><td>GLA [114]</td><td>\( \mathop{\sum }\limits_{{j = 1}}^{t}\left( {{\mathbf{q}}_{t}^{\top }\left( {\mathop{\prod }\limits_{{s = j + 1}}^{t}\operatorname{Diag}\left( {\mathbf{\alpha }}_{s}\right) }\right) {\mathbf{k}}_{j}}\right) {\mathbf{v}}_{j} \)</td><td>\( \left( {\left( {\mathbf{Q} \odot  \mathbf{\Gamma }}\right) {\left( \frac{\mathbf{K}}{\mathbf{\Gamma }}\right) }^{\top } \odot  \mathbf{M}}\right) \mathbf{V} \)</td></tr><tr><td>DeltaNet [84]</td><td>\( \mathop{\sum }\limits_{{j = 1}}^{t}\left( {{\mathbf{q}}_{t}^{\top }\left( {\mathop{\prod }\limits_{{s = j + 1}}^{t}\left( {\mathbf{I} - {\mathbf{k}}_{s}{\mathbf{k}}_{s}^{\top }}\right) }\right) {\mathbf{k}}_{j}}\right) {\mathbf{v}}_{j} \)</td><td>\( \left( {\mathbf{Q}{\mathbf{K}}^{\top } \odot  \mathbf{M}}\right) {\left( \mathbf{I} + \mathbf{K}{\mathbf{K}}^{\top } \odot  {\mathbf{M}}^{ - }\right) }^{-1}\mathbf{V} \)</td></tr><tr><td>FoX [58]</td><td>\( \mathop{\sum }\limits_{{j = 1}}^{t}\exp \left( {{\mathbf{q}}_{t}^{\top }{\mathbf{k}}_{j}}\right) \left( {\mathop{\prod }\limits_{{s = j + 1}}^{t}{\alpha }_{s}}\right) {\mathbf{v}}_{j} \)</td><td>\( \left( {\exp \left( {\mathbf{{QK}}}^{\top }\right)  \odot  \mathcal{A} \odot  \mathbf{M}}\right) \mathbf{V} \)</td></tr><tr><td>DeltaFormer [125]</td><td>\( \mathop{\sum }\limits_{{j = 1}}^{t}\left( {\phi {\left( {\mathbf{q}}_{t}\right) }^{\top }\left( {\mathop{\prod }\limits_{{s = j + 1}}^{t}\left( {\mathbf{I} - \phi \left( {\mathbf{k}}_{s}\right) \phi {\left( {\mathbf{w}}_{s}\right) }^{\top }}\right) }\right) \phi \left( {\mathbf{k}}_{j}\right) }\right) {\mathbf{v}}_{j} \)</td><td>\( \left( {\exp \left( {\mathbf{{QK}}}^{\top }\right)  \odot  \mathbf{M}}\right) {\left( \mathbf{I} + \exp \left( {\mathbf{{WK}}}^{\top }\right)  \odot  {\mathbf{M}}^{ - }\right) }^{-1}\mathbf{V} \)</td></tr><tr><td>PaTH-FoX [115]</td><td>\( \mathop{\sum }\limits_{{j = 1}}^{t}\exp \left( {{\mathbf{q}}_{t}^{\top }\left( {\mathop{\prod }\limits_{{s = j + 1}}^{t}\left( {\mathbf{I} - {\mathbf{w}}_{s}{\mathbf{w}}_{s}^{\top }}\right) }\right) {\mathbf{k}}_{j}}\right) \left( {\mathop{\prod }\limits_{{s = j + 1}}^{t}{\alpha }_{s}}\right) {\mathbf{v}}_{j} \)</td><td>\( \left( {\exp \left( {\left( {\mathbf{Q}{\mathbf{K}}^{\top } \odot  \mathbf{M}}\right) {\left( \mathbf{I} + \mathbf{W}{\mathbf{W}}^{\top } \odot  {\mathbf{M}}^{ - }\right) }^{-1}}\right)  \odot  \mathcal{A} \odot  \mathbf{M}}\right) \mathbf{V} \)</td></tr><tr><td>GDN [111]</td><td>\( \mathop{\sum }\limits_{{j = 1}}^{t}\left( {{\mathbf{q}}_{t}^{\top }\left( {\mathop{\prod }\limits_{{s = j + 1}}^{t}{\alpha }_{s}\left( {\mathbf{I} - {\mathbf{k}}_{s}{\mathbf{k}}_{s}^{\top }}\right) }\right) {\mathbf{k}}_{j}}\right) {\mathbf{v}}_{j} \)</td><td>\( \left( {{\mathbf{{QK}}}^{\top } \odot  \mathcal{A} \odot  \mathbf{M}}\right) {\left( \mathbf{I} + {\mathbf{{KK}}}^{\top } \odot  \mathcal{A} \odot  {\mathbf{M}}^{ - }\right) }^{-1}\mathbf{V} \)</td></tr><tr><td>Comba [40]</td><td>\( \mathop{\sum }\limits_{{j = 1}}^{t}\left( {{\mathbf{q}}_{t}^{\top }\left( {\mathop{\prod }\limits_{{s = j + 1}}^{t}\left( {{\alpha }_{s} - {\mathbf{k}}_{s}{\mathbf{k}}_{s}^{\top }}\right) }\right) {\mathbf{k}}_{j}}\right) {\mathbf{v}}_{j} \)</td><td>\( \left( {\mathbf{Q}{\mathbf{K}}^{\top } \odot  \mathcal{A} \odot  \mathbf{M}}\right) {\left( \mathbf{I} + \mathbf{K}{\mathbf{K}}^{\top } \odot  {\mathcal{A}}^{i - 1/j} \odot  {\mathbf{M}}^{ - }\right) }^{-1}\mathbf{V} \)</td></tr><tr><td>RWKV7 [71]</td><td>\( \mathop{\sum }\limits_{{j = 1}}^{t}\left( {{\mathbf{q}}_{t}^{\top }\left( {\mathop{\prod }\limits_{{s = j + 1}}^{t}\left( {\operatorname{Diag}\left( {\mathbf{\alpha }}_{s}\right)  - \left( {{\mathbf{b}}_{s} \odot  {\widehat{\mathbf{k}}}_{s}}\right) {\widehat{\mathbf{k}}}_{s}^{\top }}\right) }\right) {\mathbf{k}}_{j}}\right) {\mathbf{v}}_{j} \)</td><td>\( \left( {\left( {\mathbf{Q} \odot  \mathbf{\Gamma }}\right) {\left( \frac{\mathbf{K}}{\mathbf{\Gamma }}\right) }^{\top } \odot  \mathbf{M}}\right) {\left( \mathbf{I} + \left( \widehat{\mathbf{K}} \odot  \overset{0 \rightarrow  t - 1}{\mathbf{\Gamma }}\right) {\left( \frac{\widehat{\mathbf{K}} \odot  \mathbf{B}}{\mathbf{\Gamma }}\right) }^{\top } \odot  {\mathbf{M}}^{-1}\right) }^{-1}\mathbf{V} \)</td></tr><tr><td>KDA (ours)</td><td>\( \mathop{\sum }\limits_{{j = 1}}^{t}\left( {{\mathbf{q}}_{t}^{\top }\left( {\mathop{\prod }\limits_{{s = j + 1}}^{t}\operatorname{Diag}\left( {\mathbf{\alpha }}_{s}\right) \left( {\mathbf{I} - {\mathbf{k}}_{s}{\mathbf{k}}_{s}^{\top }}\right) }\right) {\mathbf{k}}_{j}}\right) {\mathbf{v}}_{j} \)</td><td>\( \left( {\left( {\mathbf{Q} \odot  \mathbf{\Gamma }}\right) {\left( \frac{\mathbf{K}}{\mathbf{\Gamma }}\right) }^{\top } \odot  \mathbf{M}}\right) {\left( \mathbf{I} + \left( \mathbf{K} \odot  \mathbf{\Gamma }\right) {\left( \frac{\mathbf{K}}{\mathbf{\Gamma }}\right) }^{\top } \odot  {\mathbf{M}}^{-1}\right) }^{-1}\mathbf{V} \)</td></tr></table>
...
To address this issue, KDA introduces a constrained variant of DPLR, where Eq. 1 can be rewritten as \( {\mathbf{S}}_{t} = \; \left( {\operatorname{Diag}\left( {\mathbf{\alpha }}_{t}\right)  - {\beta }_{t}{\mathbf{k}}_{t}{\mathbf{k}}_{t}^{\top }\operatorname{Diag}\left( {\mathbf{\alpha }}_{t}\right) }\right) {\mathbf{S}}_{t - 1} + {\beta }_{t}{\mathbf{k}}_{t}{\mathbf{v}}_{t}^{\top } \) with the correspondence between the two given by:

\[
{\mathbf{S}}_{t} = \left( {\mathbf{D} - {\mathbf{a}}_{t}{\mathbf{b}}_{t}^{\top }}\right) {\mathbf{S}}_{t - 1} + {\mathbf{k}}_{t}{\mathbf{v}}_{t}^{\top }\text{ , s.t., }\;\mathbf{D} = \operatorname{Diag}\left( {\mathbf{\alpha }}_{t}\right) ,{\mathbf{a}}_{t} = {\beta }_{t}{\mathbf{k}}_{t},{\mathbf{b}}_{t} = {\mathbf{k}}_{t} \odot  {\mathbf{\alpha }}_{t}\text{ . }
\]

Furthermore, by sharing \( {\mathbf{\alpha }}_{t} \) , we can factor it out as in Eq. 1, enabling a fine-grained multiplicative decay over \( {\mathbf{S}}_{t} \) in a manner similar to GLA [114], followed by a Householder-style transformation like DeltaNet [84, 112] for efficient
...
def chunk_dplr(q, k, v, a, b, g, chunk_size):
	...
	\( \hookrightarrow  \mathrm{d} \rightarrow  \mathrm{b} \) h n c \( {\mathrm{d}}^{\prime },\mathrm{c} = \mathrm{{BT}}),\left\lbrack  {\mathrm{q},\mathrm{k},\mathrm{v},\mathrm{a},\mathrm{b},\mathrm{g}}\right\rbrack  ) \)

	gc = g.cumsum(-2)

	Aab, Aak, Aqb, Aqk = (torch.zeros(B, H, NT, BT, BT) for
	...
	for i in range(0, NT):
		...
		\( \hookrightarrow  \left( {q, k, v, u, w, b}\right) ) \)
...
(a) PyTorch-style pseudo code for chunkwise DPLR. state updating. We provide a side-by-side comparison of the chunkwise PyTorch-style pseudocode implementations for DPLR and KDA in Listing 8a and Listing 8b.The key improvements are highlighted below:

---

ef chunk_kda(q, k, v, a, b, g, chunk_size):

	B, H, T, K, V, BT = *q.shape, v.shape[-1], chunk_size

	NT, S = T // BT, k.new_zeros(B, H, K, V)

	q, k, v, g = map(lambda x: rearrange(x, 'bh (n c) ...

	\( \hookrightarrow   \rightarrow \) b h n c ...', c=BT), [q, k, v, g])
	...
	for i in range(1, BT):
			...
			\( A\left\lbrack  {\ldots , i, : i}\right\rbrack   = A\left\lbrack  {\ldots , i, : i}\right\rbrack   + (A\left\lbrack  {\ldots , i, : ,\text{ None }}\right\rbrack \)

			\( \hookrightarrow \) * A[...,:,:i].clone()).sum(-2)

	\( A = \left( {A + \text{ torch.eye(BT)) }}\right) \)

	\( w, u = A@\left( {g\left( {c,\exp \left( \right)  * k}\right) }\right) , A@v \)
...
<table><tr><td></td><td>Objective \( \mathcal{L} \)</td><td>Update rule \( {\mathbf{S}}_{t} = {\mathbf{S}}_{t - 1} - {\nabla }_{{\mathbf{S}}_{t - 1}}\mathcal{L} \)</td></tr><tr><td>LA [48]</td><td>\( - \left\langle  {{\mathbf{S}}_{t - 1}^{\top }{\mathbf{k}}_{t},{\mathbf{v}}_{t}}\right\rangle \)</td><td>\( {\mathbf{S}}_{t} = {\mathbf{S}}_{t - 1} + {\mathbf{k}}_{t}{\mathbf{v}}_{t}^{\top } \)</td></tr><tr><td>RetNet [92]</td><td>\( - {\beta }_{t}\left\langle  {{\mathbf{S}}_{t - 1}^{\top }{\mathbf{k}}_{t},{\mathbf{v}}_{t}}\right\rangle   + \frac{1}{2}{\begin{Vmatrix}\sqrt{1 - \alpha }{\mathbf{S}}_{t - 1}\end{Vmatrix}}_{F}^{2} \)</td><td>\( {\mathbf{S}}_{t} = \alpha {\mathbf{S}}_{t - 1} + {\beta }_{t}{\mathbf{k}}_{t}{\mathbf{v}}_{t}^{\top } \)</td></tr><tr><td>Mamba2 [16]</td><td>\( - {\beta }_{t}\left\langle  {{\mathbf{S}}_{t - 1}^{\top }{\mathbf{k}}_{t},{\mathbf{v}}_{t}}\right\rangle   + \frac{1}{2}{\begin{Vmatrix}\sqrt{1 - {\alpha }_{t}}{\mathbf{S}}_{t - 1}\end{Vmatrix}}_{F}^{2} \)</td><td>\( {\mathbf{S}}_{t} = {\alpha }_{t}{\mathbf{S}}_{t - 1} + {\beta }_{t}{\mathbf{k}}_{t}{\mathbf{v}}_{t}^{\top } \)</td></tr><tr><td>GLA [114]</td><td>\( - \left\langle  {{\mathbf{S}}_{t - 1}^{\top }{\mathbf{k}}_{t},{\mathbf{v}}_{t}}\right\rangle   + \frac{1}{2}{\begin{Vmatrix}\sqrt{\operatorname{Diag}\left( {\mathbf{1} - {\mathbf{\alpha }}_{t}}\right) }{\mathbf{S}}_{t - 1}\end{Vmatrix}}_{F}^{2} \)</td><td>\( {\mathbf{S}}_{t} = \operatorname{Diag}\left( {\mathbf{\alpha }}_{t}\right) {\mathbf{S}}_{t - 1} + {\mathbf{k}}_{t}{\mathbf{v}}_{t}^{\top } \)</td></tr><tr><td>HGRN2 [77]</td><td>\( - \left\langle  {{\mathbf{S}}_{t - 1}^{\top }\left( {\mathbf{1} - {\mathbf{\alpha }}_{t}}\right) ,{\mathbf{v}}_{t}}\right\rangle   + \frac{1}{2}{\begin{Vmatrix}\sqrt{\operatorname{Diag}\left( {\mathbf{1} - {\mathbf{\alpha }}_{t}}\right) }{\mathbf{S}}_{t - 1}\end{Vmatrix}}_{F}^{2} \)</td><td>\( {\mathbf{S}}_{t} = \operatorname{Diag}\left( {\mathbf{\alpha }}_{t}\right) {\mathbf{S}}_{t - 1} + \left( {\mathbf{1} - {\mathbf{\alpha }}_{t}}\right) {\mathbf{v}}_{t}^{\top } \)</td></tr><tr><td>Longhorn [59]</td><td>\( \frac{1}{2}{\begin{Vmatrix}{\mathbf{S}}_{t - 1}^{\top }{\mathbf{k}}_{t} - {\mathbf{v}}_{t}\end{Vmatrix}}_{\text{ Diag }\left( {\mathbf{\beta }}_{t}\right) }^{2} \)</td><td>\( {\mathbf{S}}_{t} = \left( {\mathbf{I} - \frac{{\beta }_{t}}{1 + {\beta }_{t}{\mathbf{k}}_{t}^{\mathrm{T}}{\mathbf{k}}_{t}}{\mathbf{k}}_{t}{\mathbf{k}}_{t}^{\mathrm{T}}}\right) {\mathbf{S}}_{t - 1} + {\beta }_{t}{\mathbf{k}}_{t}{\mathbf{v}}_{t}^{\mathrm{T}} \)</td></tr><tr><td>Comba [40]</td><td>\( \frac{{\beta }_{t}}{2}{\begin{Vmatrix}{\mathbf{S}}_{t - 1}^{\top }{\mathbf{k}}_{t} - {\mathbf{v}}_{t}\end{Vmatrix}}^{2} + \frac{1}{2}{\begin{Vmatrix}\sqrt{1 - {\alpha }_{t}}{\mathbf{S}}_{t - 1}\end{Vmatrix}}_{F}^{2} \)</td><td>\( {\mathbf{S}}_{t} = \left( {{\alpha }_{t} - {\beta }_{t}{\mathbf{k}}_{t}{\widehat{\mathbf{k}}}_{t}^{\top }}\right) {\mathbf{S}}_{t - 1} + {\beta }_{t}{\mathbf{k}}_{t}{\mathbf{v}}_{t}^{\top } \)</td></tr><tr><td>RWKV7 [71]</td><td>\( \frac{1}{2}{\begin{Vmatrix}{\mathbf{S}}_{t - 1}^{\top }{\widetilde{\mathbf{k}}}_{t} - {\mathbf{v}}_{t}\end{Vmatrix}}^{2} + \frac{1}{2}{\begin{Vmatrix}\sqrt{\operatorname{Diag}\left( {\mathbf{1} - {\mathbf{\alpha }}_{t}}\right) }{\mathbf{S}}_{t - 1}\end{Vmatrix}}_{F}^{2} \)</td><td>\( {\mathbf{S}}_{t} = \left( {\operatorname{Diag}\left( {\mathbf{\alpha }}_{t}\right)  - \left( {{\mathbf{b}}_{s} \odot  {\widehat{\mathbf{k}}}_{s}}\right) {\widehat{\mathbf{k}}}_{t}^{\top }}\right) {\mathbf{S}}_{t - 1} + {\mathbf{k}}_{t}{\mathbf{v}}_{t}^{\top } \)</td></tr><tr><td>GDN [111]</td><td></td><td>\( {\mathbf{S}}_{t} = \left( {\mathbf{I} - {\beta }_{t}{\mathbf{k}}_{t}{\mathbf{k}}_{t}^{\top }}\right) {\alpha }_{t}{\mathbf{S}}_{t - 1} + {\beta }_{t}{\mathbf{k}}_{t}{\mathbf{v}}_{t}^{\top } \)</td></tr><tr><td>KDA (ours)</td><td>\( \begin{array}{l} \frac{{\beta }_{t}}{2}{\begin{Vmatrix}{\widetilde{\mathbf{S}}}_{t - 1}^{\top }{\mathbf{k}}_{t} - {\mathbf{v}}_{t}\end{Vmatrix}}^{2} \\  \frac{{\beta }_{t}}{2}{\begin{Vmatrix}{\widetilde{\mathbf{S}}}_{t - 1}^{\top }{\mathbf{k}}_{t} - {\mathbf{v}}_{t}\end{Vmatrix}}^{2} \end{array} \)</td><td>\( {\mathbf{S}}_{t} = \left( {\mathbf{I} - {\beta }_{t}{\mathbf{k}}_{t}{\mathbf{k}}_{t}^{\top }}\right) \operatorname{Diag}\left( {\mathbf{\alpha }}_{t}\right) {\mathbf{S}}_{t - 1} + {\beta }_{t}{\mathbf{k}}_{t}{\mathbf{v}}_{t}^{\top } \)</td></tr></table>

For GDN and KDA, the update can be viewed as performing an Stochastic Gradient Descent(SGD) process on the decayed state \( \widetilde{\mathbf{S}} \) , that is, \( {\mathbf{S}}_{t} = \; {\widetilde{\mathbf{S}}}_{t - 1} - {\nabla }_{{\overline{\mathbf{S}}}_{t - 1}}\mathcal{L} \) , where \( {\widetilde{\mathbf{S}}}_{t - 1} \) is decayed by scalar or fine-grained gate.

§ 3.1), while switching to the more efficient recurrent kernel (Eq. 2) for autoregressive generation. A key advantage of the Linear KDA is its ability to maintain a fixed-sized state \( \left( {{d}_{k} \times  {d}_{v}}\right. \) per head, with \( {d}_{k} = {d}_{v} = {128} \) ) regardless of sequence length. For our hybrid model, as sequence length increases, the I/O-bounded decoding time approaches a maximum hybrid efficiency ratio of 3:1 compared to full attention. This trend is reflected in Fig. 7b, where Kimi Linear achieves a \( {2.3} \times \) speedup at a 1M token context. Additionally, by eliminating the need for a large, linear-scaling KV cache, Kimi Linear is able to reallocate memory resources to support larger batch sizes, enhancing overall throughput. In long-context scenarios (up to 1M tokens), this memory efficiency results in a theoretical decoding speedup of up to 6.3×(see Fig. 1b).
...
[73] Piotr Piekos, Róbert Csordás, and Jürgen Schmidhuber. "Mixture of Sparse Attention: Content-Based Learnable Sparse Attention via Expert-Choice Routing". In: arXiv preprint arXiv:2505.00315 (2025).
...
[105] Colin White et al. "Livebench: A challenging, contamination-free llm benchmark". In: arXiv preprint arXiv:2406.193144 (2024).
...
[116] Feng Yao et al. Your Efficient RL Framework Secretly Brings You Off-Policy RL Training. Aug. 2025. URL: https://fengyao.notion.site/off-policy-r1.
...
\[
{\mathbf{S}}_{\left\lbrack  t\right\rbrack  }^{r} = \underset{ \mathrel{\text{ := }} {\mathbf{P}}_{\left\lbrack  t\right\rbrack  }^{r}}{\underbrace{\left( \mathop{\prod }\limits_{{i = 1}}^{r}\left( \mathbf{I} - {\beta }_{\left\lbrack  t\right\rbrack  }^{i}{\mathbf{k}}_{\left\lbrack  t\right\rbrack  }^{i}{\mathbf{k}}_{\left\lbrack  t\right\rbrack  }^{i\top }\right) \operatorname{Diag}\left( {\mathbf{\alpha }}_{\left\lbrack  t\right\rbrack  }^{i}\right) \right) }} \cdot  {\mathbf{S}}_{\left\lbrack  t\right\rbrack  }^{0} + \underset{ \mathrel{\text{ := }} {\mathbf{H}}_{\left\lbrack  t\right\rbrack  }^{r}}{\underbrace{\mathop{\sum }\limits_{{i = 1}}^{r}\left( {\mathop{\prod }\limits_{{j = i + 1}}^{r}\left( {\mathbf{I} - {\beta }_{\left\lbrack  t\right\rbrack  }^{j}{\mathbf{k}}_{\left\lbrack  t\right\rbrack  }^{j}{\mathbf{k}}_{\left\lbrack  t\right\rbrack  }^{j\top }}\right) \operatorname{Diag}\left( {\mathbf{\alpha }}_{\left\lbrack  t\right\rbrack  }^{j}\right) }\right)  \cdot  {\beta }_{\left\lbrack  t\right\rbrack  }^{i}{\mathbf{k}}_{\left\lbrack  t\right\rbrack  }^{i}{\mathbf{v}}_{\left\lbrack  t\right\rbrack  }^{i\top }}}
\]

\[
= {\mathbf{P}}_{\left\lbrack  t\right\rbrack  }^{r} \cdot  {\mathbf{S}}_{\left\lbrack  t\right\rbrack  }^{0} + {\mathbf{H}}_{\left\lbrack  t\right\rbrack  }^{r}
\]

Our goal is to transform \( {\mathbf{P}}_{\left\lbrack  t\right\rbrack  }^{r} \) and \( {\mathbf{H}}_{\left\lbrack  t\right\rbrack  }^{r} \) into matrix forms suitable for parallel computation.

We show that \( {\mathbf{P}}_{\left\lbrack  t\right\rbrack  }^{r} \) , which involves the cumulative product of generalized Householder matrices, can be optimized using the classic WY representation.

Proposition 1. The matrix \( {\mathbf{P}}_{\left\lbrack  t\right\rbrack  }^{r} \) can be expressed as:

\[
{\mathbf{P}}_{\left\lbrack  t\right\rbrack  }^{r} = \operatorname{Diag}\left( {\gamma }_{\left\lbrack  t\right\rbrack  }^{r}\right)  - \mathop{\sum }\limits_{{i = 1}}^{r}\operatorname{Diag}\left( {\gamma }_{\left\lbrack  t\right\rbrack  }^{i \rightarrow  r}\right) {\mathbf{k}}_{\left\lbrack  t\right\rbrack  }^{i}{\mathbf{w}}_{\left\lbrack  t\right\rbrack  }^{i\top } \tag{16}
\]

where the auxiliary vector \( {\mathbf{w}}_{\left\lbrack  t\right\rbrack  }^{r} \in  {\mathbb{R}}^{{d}_{k}} \) is computed via the following recurrence relation:

\[
{\mathbf{w}}_{\left\lbrack  t\right\rbrack  }^{r} = {\beta }_{\left\lbrack  t\right\rbrack  }^{r}\left( {\operatorname{Diag}\left( {\gamma }_{\left\lbrack  t\right\rbrack  }^{r}\right) {\mathbf{k}}_{\left\lbrack  t\right\rbrack  }^{r} - \mathop{\sum }\limits_{{i = 1}}^{{r - 1}}{\mathbf{w}}_{\left\lbrack  t\right\rbrack  }^{i}\left( {{\mathbf{k}}_{\left\lbrack  t\right\rbrack  }^{i\top }\operatorname{Diag}\left( {\gamma }_{\left\lbrack  t\right\rbrack  }^{i \rightarrow  r}\right) {\mathbf{k}}_{\left\lbrack  t\right\rbrack  }^{r}}\right) }\right) \tag{17}
\]

Proof. We proceed with a proof by mathematical induction.

Inductive Step: Assume the proposition holds for \( r - 1 \) , i.e., \( {\mathbf{P}}_{\left\lbrack  t\right\rbrack  }^{r - 1} = \operatorname{Diag}\left( {\mathbf{\gamma }}_{\left\lbrack  t\right\rbrack  }^{r - 1}\right)  - \mathop{\sum }\limits_{{i = 1}}^{{r - 1}}\operatorname{Diag}\left( {\mathbf{\gamma }}_{\left\lbrack  t\right\rbrack  }^{i \rightarrow  r - 1}\right) {\mathbf{k}}_{\left\lbrack  t\right\rbrack  }^{i}{\mathbf{w}}_{\left\lbrack  t\right\rbrack  }^{i\top } \) . We now derive:

\[
{\mathbf{P}}_{\left\lbrack  t\right\rbrack  }^{r} = \left( {\mathbf{I} - {\beta }_{\left\lbrack  t\right\rbrack  }^{r}{\mathbf{k}}_{\left\lbrack  t\right\rbrack  }^{r}{\mathbf{k}}_{\left\lbrack  t\right\rbrack  }^{r\top }}\right) \operatorname{Diag}\left( {\mathbf{\alpha }}_{\left\lbrack  t\right\rbrack  }^{r}\right) {\mathbf{P}}_{\left\lbrack  t\right\rbrack  }^{r - 1}
\]

\[
= \left( {\mathbf{I} - {\beta }_{\left\lbrack  t\right\rbrack  }^{r}{\mathbf{k}}_{\left\lbrack  t\right\rbrack  }^{r}{\mathbf{k}}_{\left\lbrack  t\right\rbrack  }^{r\top }}\right) \operatorname{Diag}\left( {\mathbf{\alpha }}_{\left\lbrack  t\right\rbrack  }^{r}\right) \left( {\operatorname{Diag}\left( {\mathbf{\gamma }}_{\left\lbrack  t\right\rbrack  }^{r - 1}\right)  - \mathop{\sum }\limits_{{i = 1}}^{{r - 1}}\operatorname{Diag}\left( {\mathbf{\gamma }}_{\left\lbrack  t\right\rbrack  }^{i \rightarrow  r - 1}\right) {\mathbf{k}}_{\left\lbrack  t\right\rbrack  }^{i}{\mathbf{w}}_{\left\lbrack  t\right\rbrack  }^{i\top }}\right)
\]

\[
= \left( {\mathbf{I} - {\beta }_{\left\lbrack  t\right\rbrack  }^{r}{\mathbf{k}}_{\left\lbrack  t\right\rbrack  }^{r}{\mathbf{k}}_{\left\lbrack  t\right\rbrack  }^{r\top }}\right) \left( {\operatorname{Diag}\left( {\mathbf{\gamma }}_{\left\lbrack  t\right\rbrack  }^{r}\right)  - \mathop{\sum }\limits_{{i = 1}}^{{r - 1}}\operatorname{Diag}\left( {\mathbf{\gamma }}_{\left\lbrack  t\right\rbrack  }^{i \rightarrow  r}\right) {\mathbf{k}}_{\left\lbrack  t\right\rbrack  }^{i}{\mathbf{w}}_{\left\lbrack  t\right\rbrack  }^{i\top }}\right)
\]

\[
= \operatorname{Diag}\left( {\mathbf{\gamma }}_{\left\lbrack  t\right\rbrack  }^{r}\right)  - \mathop{\sum }\limits_{{i = 1}}^{{r - 1}}\operatorname{Diag}\left( {\mathbf{\gamma }}_{\left\lbrack  t\right\rbrack  }^{i \rightarrow  r}\right) {\mathbf{k}}_{\left\lbrack  t\right\rbrack  }^{i}{\mathbf{w}}_{\left\lbrack  t\right\rbrack  }^{i\top } - {\mathbf{\beta }}_{\left\lbrack  t\right\rbrack  }^{r}{\mathbf{k}}_{\left\lbrack  t\right\rbrack  }^{r}{\mathbf{k}}_{\left\lbrack  t\right\rbrack  }^{r\top }\operatorname{Diag}\left( {\mathbf{\gamma }}_{\left\lbrack  t\right\rbrack  }^{r}\right)  + {\mathbf{\beta }}_{\left\lbrack  t\right\rbrack  }^{r}{\mathbf{k}}_{\left\lbrack  t\right\rbrack  }^{r}{\mathbf{k}}_{\left\lbrack  t\right\rbrack  }^{r\top }\mathop{\sum }\limits_{{i = 1}}^{{r - 1}}\operatorname{Diag}\left( {\mathbf{\gamma }}_{\left\lbrack  t\right\rbrack  }^{i \rightarrow  r}\right) {\mathbf{k}}_{\left\lbrack  t\right\rbrack  }^{i}{\mathbf{w}}_{\left\lbrack  t\right\rbrack  }^{i\top }
\]

\[
= \operatorname{Diag}\left( {\mathbf{\gamma }}_{\left\lbrack  t\right\rbrack  }^{r}\right)  - \mathop{\sum }\limits_{{i = 1}}^{{r - 1}}\operatorname{Diag}\left( {\mathbf{\gamma }}_{\left\lbrack  t\right\rbrack  }^{i \rightarrow  r}\right) {\mathbf{k}}_{\left\lbrack  t\right\rbrack  }^{i}{\mathbf{w}}_{\left\lbrack  t\right\rbrack  }^{i\top } - {\mathbf{k}}_{\left\lbrack  t\right\rbrack  }^{r}{\left( {\beta }_{\left\lbrack  t\right\rbrack  }^{r}\operatorname{Diag}\left( {\mathbf{\gamma }}_{\left\lbrack  t\right\rbrack  }^{r}\right) {\mathbf{k}}_{\left\lbrack  t\right\rbrack  }^{r}\right) }^{\top } + {\mathbf{k}}_{\left\lbrack  t\right\rbrack  }^{r}{\left( {\beta }_{\left\lbrack  t\right\rbrack  }^{r}\mathop{\sum }\limits_{{i = 1}}^{{r - 1}}{\mathbf{w}}_{\left\lbrack  t\right\rbrack  }^{i}\left( {\mathbf{k}}_{\left\lbrack  t\right\rbrack  }^{i\top }\operatorname{Diag}\left( {\mathbf{\gamma }}_{\left\lbrack  t\right\rbrack  }^{i \rightarrow  r}\right) {\mathbf{k}}_{\left\lbrack  t\right\rbrack  }^{r}\right) \right) }^{\top }
\]

\[
= \operatorname{Diag}\left( {\mathbf{\gamma }}_{\left\lbrack  t\right\rbrack  }^{r}\right)  - \mathop{\sum }\limits_{{i = 1}}^{{r - 1}}\operatorname{Diag}\left( {\mathbf{\gamma }}_{\left\lbrack  t\right\rbrack  }^{i \rightarrow  r}\right) {\mathbf{k}}_{\left\lbrack  t\right\rbrack  }^{i}{\mathbf{w}}_{\left\lbrack  t\right\rbrack  }^{i\top } - {\mathbf{k}}_{\left\lbrack  t\right\rbrack  }^{r}\underset{{\mathbf{w}}_{\left\lbrack  t\right\rbrack  }^{r}}{\underbrace{{\left( {\beta }_{\left\lbrack  t\right\rbrack  }^{r}\left( \operatorname{Diag}\left( {\mathbf{\gamma }}_{\left\lbrack  t\right\rbrack  }^{r}\right) {\mathbf{k}}_{\left\lbrack  t\right\rbrack  }^{r} - \mathop{\sum }\limits_{{i = 1}}^{{r - 1}}{\mathbf{w}}_{\left\lbrack  t\right\rbrack  }^{i}\left( {\mathbf{k}}_{\left\lbrack  t\right\rbrack  }^{i\top }\right) \operatorname{Diag}\left( {\mathbf{\gamma }}_{\left\lbrack  t\right\rbrack  }^{i \rightarrow  r}{\mathbf{k}}_{\left\lbrack  t\right\rbrack  }^{r}\right) \right) \right) }^{\top }}}
\]

\[
= \operatorname{Diag}\left( {\mathbf{\gamma }}_{\left\lbrack  t\right\rbrack  }^{r}\right)  - \mathop{\sum }\limits_{{i = 1}}^{{r - 1}}\operatorname{Diag}\left( {\mathbf{\gamma }}_{\left\lbrack  t\right\rbrack  }^{i \rightarrow  r}\right) {\mathbf{k}}_{\left\lbrack  t\right\rbrack  }^{i}{\mathbf{w}}_{\left\lbrack  t\right\rbrack  }^{i\top } - {\mathbf{k}}_{\left\lbrack  t\right\rbrack  }^{r}{\mathbf{w}}_{\left\lbrack  t\right\rbrack  }^{r\top }
\]

\[
= \operatorname{Diag}\left( {\mathbf{\gamma }}_{\left\lbrack  t\right\rbrack  }^{r}\right)  - \mathop{\sum }\limits_{{i = 1}}^{r}\operatorname{Diag}\left( {\mathbf{\gamma }}_{\left\lbrack  t\right\rbrack  }^{i \rightarrow  r}\right) {\mathbf{k}}_{\left\lbrack  t\right\rbrack  }^{i}{\mathbf{w}}_{\left\lbrack  t\right\rbrack  }^{i\top }
\]

The inductive step holds.

Similar to \( {\mathbf{P}}_{\left\lbrack  t\right\rbrack  }^{r},{\mathbf{H}}_{\left\lbrack  t\right\rbrack  }^{r} \) can also be expressed in a parallelizable form.

Proposition 2. The matrix \( {\mathbf{H}}_{\left\lbrack  t\right\rbrack  }^{r} \) can be expressed as:

\[
{\mathbf{H}}_{\left\lbrack  t\right\rbrack  }^{r} = \mathop{\sum }\limits_{{i = 1}}^{r}\operatorname{Diag}\left( {\gamma }_{\left\lbrack  t\right\rbrack  }^{i \rightarrow  r}\right) {\mathbf{k}}_{\left\lbrack  t\right\rbrack  }^{i}{\mathbf{u}}_{\left\lbrack  t\right\rbrack  }^{i\top } \tag{18}
\]

where the auxiliary vector \( {\mathbf{u}}_{\left\lbrack  t\right\rbrack  }^{r} \in  {\mathbb{R}}^{{d}_{v}} \) is computed via the following recurrence relation:

\[
{\mathbf{u}}_{\left\lbrack  t\right\rbrack  }^{r} = {\beta }_{\left\lbrack  t\right\rbrack  }^{r}\left( {{\mathbf{v}}_{\left\lbrack  t\right\rbrack  }^{r} - \mathop{\sum }\limits_{{i = 1}}^{{r - 1}}{\mathbf{u}}_{\left\lbrack  t\right\rbrack  }^{i}\left( {{\mathbf{k}}_{\left\lbrack  t\right\rbrack  }^{i\top }\operatorname{Diag}\left( {\gamma }_{\left\lbrack  t\right\rbrack  }^{i \rightarrow  r}\right) {\mathbf{k}}_{\left\lbrack  t\right\rbrack  }^{r}}\right) }\right) \tag{19}
\]
...
\[
{\mathbf{H}}_{\left\lbrack  t\right\rbrack  }^{r} = \left( {\mathbf{I} - {\beta }_{\left\lbrack  t\right\rbrack  }^{r}{\mathbf{k}}_{\left\lbrack  t\right\rbrack  }^{r}{\mathbf{k}}_{\left\lbrack  t\right\rbrack  }^{r\top }}\right) \operatorname{Diag}\left( {\mathbf{\alpha }}_{\left\lbrack  t\right\rbrack  }^{r}\right) {\mathbf{H}}_{\left\lbrack  t\right\rbrack  }^{r - 1} + {\beta }_{\left\lbrack  t\right\rbrack  }^{r}{\mathbf{k}}_{\left\lbrack  t\right\rbrack  }^{r}{\mathbf{v}}_{\left\lbrack  t\right\rbrack  }^{r\top }
\]

\[
= \left( {\mathbf{I} - {\beta }_{\left\lbrack  t\right\rbrack  }^{r}{\mathbf{k}}_{\left\lbrack  t\right\rbrack  }^{r}{\mathbf{k}}_{\left\lbrack  t\right\rbrack  }^{r\top }}\right) \operatorname{Diag}\left( {\mathbf{\alpha }}_{\left\lbrack  t\right\rbrack  }^{r}\right) \left( {\mathop{\sum }\limits_{{i = 1}}^{{r - 1}}\operatorname{Diag}\left( {\mathbf{\gamma }}_{\left\lbrack  t\right\rbrack  }^{i \rightarrow  r - 1}\right) {\mathbf{k}}_{\left\lbrack  t\right\rbrack  }^{i}{\mathbf{u}}_{\left\lbrack  t\right\rbrack  }^{i\top }}\right)  + {\beta }_{\left\lbrack  t\right\rbrack  }^{r}{\mathbf{k}}_{\left\lbrack  t\right\rbrack  }^{r}{\mathbf{v}}_{\left\lbrack  t\right\rbrack  }^{r\top }
\]

\[
= \left( {\mathbf{I} - {\beta }_{\left\lbrack  t\right\rbrack  }^{r}{\mathbf{k}}_{\left\lbrack  t\right\rbrack  }^{r}{\mathbf{k}}_{\left\lbrack  t\right\rbrack  }^{r\top }}\right) \left( {\mathop{\sum }\limits_{{i = 1}}^{{r - 1}}\operatorname{Diag}\left( {{\mathbf{\gamma }}_{\left\lbrack  t\right\rbrack  }^{i \rightarrow  r}{}^{r}}\right) {\mathbf{k}}_{\left\lbrack  t\right\rbrack  }^{i}{\mathbf{u}}_{\left\lbrack  t\right\rbrack  }^{i\top }}\right)  + {\beta }_{\left\lbrack  t\right\rbrack  }^{r}{\mathbf{k}}_{\left\lbrack  t\right\rbrack  }^{r}{\mathbf{v}}_{\left\lbrack  t\right\rbrack  }^{r\top }
\]

\[
= \mathop{\sum }\limits_{{i = 1}}^{{r - 1}}\operatorname{Diag}\left( {\mathbf{\gamma }}_{\left\lbrack  t\right\rbrack  }^{i \rightarrow  r}\right) {\mathbf{k}}_{\left\lbrack  t\right\rbrack  }^{i}{\mathbf{u}}_{\left\lbrack  t\right\rbrack  }^{i\top } - {\beta }_{\left\lbrack  t\right\rbrack  }^{r}{\mathbf{k}}_{\left\lbrack  t\right\rbrack  }^{r}{\mathbf{k}}_{\left\lbrack  t\right\rbrack  }^{r\top }\mathop{\sum }\limits_{{i = 1}}^{{r - 1}}\operatorname{Diag}\left( {\mathbf{\gamma }}_{\left\lbrack  t\right\rbrack  }^{i \rightarrow  r}\right) {\mathbf{k}}_{\left\lbrack  t\right\rbrack  }^{i}{\mathbf{u}}_{\left\lbrack  t\right\rbrack  }^{i\top } + {\beta }_{\left\lbrack  t\right\rbrack  }^{r}{\mathbf{k}}_{\left\lbrack  t\right\rbrack  }^{r}{\mathbf{v}}_{\left\lbrack  t\right\rbrack  }^{r\top }
\]

\[
= \mathop{\sum }\limits_{{i = 1}}^{{r - 1}}\operatorname{Diag}\left( {\mathbf{\gamma }}_{\left\lbrack  t\right\rbrack  }^{i \rightarrow  r}\right) {\mathbf{k}}_{\left\lbrack  t\right\rbrack  }^{i}{\mathbf{u}}_{\left\lbrack  t\right\rbrack  }^{i\top } - {\mathbf{k}}_{\left\lbrack  t\right\rbrack  }^{r}{\left( {\beta }_{\left\lbrack  t\right\rbrack  }^{r}\mathop{\sum }\limits_{{i = 1}}^{{r - 1}}\left( {\mathbf{k}}_{\left\lbrack  t\right\rbrack  }^{r\top }\operatorname{Diag}\left( {\mathbf{\gamma }}_{\left\lbrack  t\right\rbrack  }^{i \rightarrow  r}\right) {\mathbf{k}}_{\left\lbrack  t\right\rbrack  }^{i}\right) {\mathbf{u}}_{\left\lbrack  t\right\rbrack  }^{i}\right) }^{\top } + {\mathbf{k}}_{\left\lbrack  t\right\rbrack  }^{r}{\beta }_{\left\lbrack  t\right\rbrack  }^{r}{\mathbf{v}}_{\left\lbrack  t\right\rbrack  }^{r\top }
\]

\[
= \mathop{\sum }\limits_{{i = 1}}^{{r - 1}}\operatorname{Diag}\left( {\mathbf{\gamma }}_{\left\lbrack  t\right\rbrack  }^{i \rightarrow  r}\right) {\mathbf{k}}_{\left\lbrack  t\right\rbrack  }^{i}{\mathbf{u}}_{\left\lbrack  t\right\rbrack  }^{i\top } + {\mathbf{k}}_{\left\lbrack  t\right\rbrack  }^{r}{\left( \underset{{\mathbf{u}}_{\left\lbrack  t\right\rbrack  }^{r}}{\underbrace{{\beta }_{\left\lbrack  t\right\rbrack  }^{r}\left( {{\mathbf{v}}_{\left\lbrack  t\right\rbrack  }^{r} - \mathop{\sum }\limits_{{i = 1}}^{{r - 1}}{\mathbf{u}}_{\left\lbrack  t\right\rbrack  }^{i}\left( {{\mathbf{k}}_{\left\lbrack  t\right\rbrack  }^{i\top }\operatorname{Diag}\left( {\mathbf{\gamma }}_{\left\lbrack  t\right\rbrack  }^{i \rightarrow  r}\right) {\mathbf{k}}_{\left\lbrack  t\right\rbrack  }^{r}}\right) }\right) }}\right) }^{\top }
\]

\[
= \mathop{\sum }\limits_{{i = 1}}^{{r - 1}}\operatorname{Diag}\left( {\mathbf{\gamma }}_{\left\lbrack  t\right\rbrack  }^{i \rightarrow  r}\right) {\mathbf{k}}_{\left\lbrack  t\right\rbrack  }^{i}{\mathbf{u}}_{\left\lbrack  t\right\rbrack  }^{i\top } + {\mathbf{k}}_{\left\lbrack  t\right\rbrack  }^{r}{\mathbf{u}}_{\left\lbrack  t\right\rbrack  }^{r\top }
\]

\[
= \mathop{\sum }\limits_{{i = 1}}^{r}\operatorname{Diag}\left( {\mathbf{\gamma }}_{\left\lbrack  t\right\rbrack  }^{i \rightarrow  r}\right) {\mathbf{k}}_{\left\lbrack  t\right\rbrack  }^{i}{\mathbf{u}}_{\left\lbrack  t\right\rbrack  }^{i\top }
\]
...
def chunk_kda(   )
	...
	for i in range(C):
		...
		\( A\left\lbrack  {\ldots , i}\right\rbrack   = \) torch.einsum('... c d,... d -> ... c', k * (g - g_i).exp(), k_i)
	...
	for i in range \( \left( {1, C}\right) \) :

		\( A\left\lbrack  {\ldots , i, i}\right\rbrack   = A\left\lbrack  {\ldots , i, i}\right\rbrack  . \) clone \( \left( \right)  + \left( {A\left\lbrack  {\ldots , i, : ,{None}}\right\rbrack  .\text{ clone }\left( \right) }\right)  * A\lbrack \ldots , : , \)

		\( \hookrightarrow \) :i].clone()).sum(-2)

	\( A = \left( {A + \text{ torch.eye(C, dtype=torch.float, device=q.device)) } * \text{ beta[..., None,:] }}\right) \)

	\( w = A@\left( {g.\exp \left( \right)  * k}\right) \)

	\( \mathrm{u} = \mathrm{A}@\mathrm{v} \)
	...
	for i in range \( \left( {0, N}\right) \) :
		...
		for j in range(C):
			...
			\( A\left\lbrack  {\ldots , j}\right\rbrack   = \) torch.einsum('... c d,... d -> ... c', q_i * (g_i - g_j).exp(), k_j)
		...
		\( S = S * \) rearrange \( \left( \left( {{g}_{ - }i\left\lbrack  { : , : , - 1}\right\rbrack  .{exp}\left( \right) ,{}^{\prime }{bhk} :  \rightarrow  {bhk}{1}^{\prime }}\right) \right) \)

		S += rearrange((g_i[:, :, -1:] - g_i).exp() * k_i, 'b h c k -> b h k c') @ v_i

	return rearrange(o, 'bh n c d -> b (n c) h d').to(dtype)
